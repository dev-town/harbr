...
export function createBrowseActions(set: TuiStoreSet, get: TuiStoreGet): Pick<
  TuiStoreActions,
  | 'changeBrowseQuery'
  | 'hoverBrowseRow'
  | 'moveBrowseSelection'
  ...
  return {
    changeBrowseQuery: (value) => {
      set((state) => ({
        ...
        browse: { ...state.browse, list: { ...state.browse.list, query: value } },
      }))
    ...
    hoverBrowseRow: (rowId) => set((state) => ({
      browse: { ...state.browse, list: { ...state.browse.list, hoveredId: rowId } },
    })),
    ...
    openBrowseActionsMenu: () => {
      ...
      set((state) => ({
        app: { ...state.app, notice: null },
        surfaces: { ...state.surfaces, surface: { kind: 'actions', route: 'browse' } },
      }))
    },
    resetBrowseQuery: () => set((state) => ({
      browse: { ...state.browse, list: { ...state.browse.list, query: '' } },
    })),
    resetBrowseSelection: () => {
      const selectedId = getRepairedSelectedId(selectVisibleBrowseRows(get()), get().browse.list.selectedId)
      set((state) => ({
        browse: { ...state.browse, list: { ...state.browse.list, hoveredId: null, selectedId } },
      }))
    },
    resetProjectScope: () => {
      set((state) => ({
    ...
    resetWorkspaceScope: () => {
      ...
      const nextScope = scope.level === 'modules' && isImplicitWorkspace(scope)
        ? projectsScope()
        : scope.level === 'modules'
    ...
    selectBrowseRow: (rowId) => set((state) => ({
      browse: { ...state.browse, list: { ...state.browse.list, selectedId: rowId } },
